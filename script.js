...
var histories = []; //record the history
...
async function GPT_api(message, user_time){
    ...
    const requestBody = {
        model: 'gpt-3.5-turbo',
        messages: [{ role: 'system', content: 'You are a helpful assistant.' }, ...histories, { role: 'user', content: message }]
    };
...
function addToHistory(input, response) {
    histories.push({ role: 'user', content: input });
    histories.push({ role: 'assistant', content: response });
}
...
const data = { user_id: "user_id", type: problem_type.value, histories: full_history };

// Function to save history to a JSON file
function saveHistoryToJson() {
  data.type = problem_type.value;
  data.histories = full_history;
...
function testing_(){
  msg = " After reading the instructions, I understand that as a Python coding tutor, I should help students in learning Python with patience. I am required to ask the user about the coding problem and then ask them which type of question they have regarding the problem. Is that correct?";
  jsPy(msg);
}